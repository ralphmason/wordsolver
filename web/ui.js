    ...
    complete: res => {
      ...
      for (var i = 0; i < cols; i++) {
        ...
        arr
          .map(
            res =>
              `<tr  class="result"><td>${res.replace(/~/g, "qu")}</td><td>${
                res.length
              }</td></tr>\r\n`
          )
          .forEach(element => {
            ...
            complete: result => {
              ...
              if (locations.length > 4) {
                ...
                switch (diff) {
                  ...
                  default:
                    if (last > second) {
                      if (last % 8 < second % 8) hichar = [6, 1, 0][val];
                      else hichar = [5, 1, 0][val];
                    } else {
                      if (last % 8 < second % 8) hichar = [5, 1, 0][val];
                      else hichar = [6, 1, 0][val];
                    }
                }
  ...
  switch (x.which) {
    ...
    case 38: //up
      while (true) {
        ...
        if (!isdisabled(id)) return focus(id);
      }
...
load.onclick = () => {
  $.ajax({
    ...
    complete: result => {
      ...
      if (lay == null) {
        alert(
          "no one has created this level yet, perhaps you could lay it out and save it for everyone to use"
        );
        return;
      }
